...
export default function App() {
  ...
  const loadLesson = async () => {
    setLessonError('');
    try {
      const response = await fetch(`${API_URL}/api/v1/users/${USER_ID}/lesson/today?level=${level}&minutes=10`);
  ...
  const send = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/conversations`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: USER_ID, message: value, level }) });
